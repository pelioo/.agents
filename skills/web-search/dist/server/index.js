...
class BridgeServer {
    constructor(config) {
        ...
        this.sharedPageQueue = Promise.resolve();
     ...
     */
    async waitForPendingBrowserLaunch() {
    ...
    async ensureBrowserReadyExclusive() {
        if (this.browserInstance) {
            const processAlive = this.isBrowserProcessAlive(this.browserInstance);
            const cdpReachable = processAlive ? await (0, browser_1.isCdpEndpointReachable)(this.browserInstance.cdpPort) : false;
        ...
        if (await (0, browser_1.isCdpEndpointReachable)(cdpPort)) {
            const probe = await (0, browser_1.probeCdpEndpoint)(cdpPort);
            if (probe.attachable) {
                console.log(`[Bridge Server] Adopting existing CDP browser on port ${cdpPort} (pid: ${probe.browserPid ?? 'unknown'})`);
                this.browserInstance = (0, browser_1.adoptBrowser)(cdpPort, probe.browserPid);
                return { instance: this.browserInstance, reused: true };
            }
            ...
            const closed = await (0, browser_1.closeUnattachableCdpBrowser)(cdpPort);
            if (!closed) {
                throw new Error(`CDP port ${cdpPort} is occupied by a browser that rejects automation clients and could not be closed; ` +
        ...
        try {
            ...
            if (error instanceof browser_1.CdpPortInUseError) {
                const probe = await (0, browser_1.probeCdpEndpoint)(cdpPort);
                if (probe.attachable) {
    ...
    async handleBrowserStatus(req, res) {
        ...
        const cdpReachable = processAlive && this.browserInstance
            ? await (0, browser_1.isCdpEndpointReachable)(this.browserInstance.cdpPort)
            : false;
        res.json({
            success: true,
            data: {
     ...
     */
    enqueueSharedPageOperation(operation) {
        const run = this.sharedPageQueue.then(operation, operation);
        this.sharedPageQueue = run.then(() => undefined, () => undefined);
        return run;
    }
    // Search operation
    async handleSearch(req, res) {
        try {
            ...
            const results = await this.enqueueSharedPageOperation(() => this.searchWithFallback(connectionId, query, maxResults, preferredEngine));
            res.json({
                success: true,
                data: results
            });
    ...
    async handleGetContent(req, res) {
        try {
            ...
            const content = await this.enqueueSharedPageOperation(() => this.bingSearch.getResultContent(connectionId, url));
            res.json({
                success: true,
                data: { content }
            });
